      ...
      <div className="space-y-4">
        {researchDirections.map((dir, idx) => {
          return (
            <div
              key={dir.id || idx}
              className="bg-[#FFFFFF] rounded-[10px] border border-[#FAF8F5] p-5 shadow-2xs space-y-4"
            >
              <div>
                <span className="text-[11px] font-mono font-semibold text-[#D97706] uppercase tracking-wider block mb-1">
                ...
                <p className="text-xs text-[#6B7280] mt-1">
                  <span className="font-semibold text-[#431407]">Target Problem:</span> {dir.problem}
                </p>
              </div>

              {/* Proposed Approach */}
              <div className="text-xs text-[#242A29] bg-[#FAF8F5] p-3.5 rounded border border-[#FAF8F5] leading-relaxed">
                <span className="font-semibold text-[#431407] block mb-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                {dir.models && dir.models.length > 0 && (
                  <div className="bg-[#FFFFFF] border border-[#FAF8F5] rounded p-3">
                    <span className="font-semibold text-[#431407] flex items-center gap-1.5 mb-1">
              ...
              {dir.methodology && dir.methodology.length > 0 && (
                <div className="pt-2">
                  <span className="font-semibold text-xs text-[#431407] flex items-center gap-1 mb-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2">
                    {dir.methodology.map((step, sIdx) => (
                      <div
                        key={sIdx}
                        className="bg-[#FAF8F5] border border-[#FAF8F5] rounded p-2.5 text-xs text-[#242A29]"
                      >
                        <span className="font-bold text-[#431407] font-mono mr-1">
                          Phase {sIdx + 1}:
                        </span>
                        <span>{step}</span>
                      </div>
