...
export const ComparisonTable: React.FC<ComparisonTableProps> = ({ papers }) => {
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.download = `inqora_literature_matrix_${Date.now()}.csv`;
  ...
  const handleExportBibTeX = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `inqora_matrix_citations_${Date.now()}.bib`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  if (papers.length === 0) {
    return (
      <div className="bg-[#FFFFFF] rounded-xl border border-[#FAF8F5] p-10 text-center text-[#6B7280]">
        ...
        <p className="text-sm font-medium text-[#431407]">No research papers available to compare.</p>
  ...
  return (
    <div className="space-y-4">
          ...
          >
            {copiedCsv ? (
              <>
                ...
                <span className="text-[#D97706] font-semibold">Exported CSV</span>
              </>
            ) : (
              <>
      ...
      <div className="bg-[#FFFFFF] rounded-[10px] border border-[#FAF8F5] shadow-[0_1px_3px_rgba(15,23,42,0.03)] overflow-hidden">
        <div className="overflow-x-auto max-h-[620px] scrollbar-thin">
          <table className="w-full text-left text-xs border-collapse">
            <thead className="sticky top-0 z-10 bg-[#EFEFEA] border-b border-[#FAF8F5] text-[#431407] shadow-xs">
              <tr className="divide-x divide-[#FAF8F5]">
              ...
              {papers.map((paper, idx) => {
                ...
                return (
                  <tr
                    ...
                    className={`divide-x divide-[#F4F4F5] cursor-pointer transition-colors ${
                      isSelected ? 'bg-[#FAF8F5]' : 'hover:bg-[#F9F9F6]'
                    }`}
                  >
                    {/* 1. Paper Title, Authors, Year, DOI */}
                    <td className="py-3.5 px-3.5 align-top">
                      <div className="font-['DM_Sans'] font-semibold text-[#431407] leading-snug">
                        {paper.title}
                      </div>
                      <div className="text-[11px] text-[#4A5568] mt-1">
                        ...
                        <span className="font-mono text-[#431407] font-medium">({paper.year || '2024'})</span>
                      </div>
                      {doi && (
                    ...
                    <td className="py-3.5 px-3.5 align-top text-[#242A29]">
                      ...
                      {paper.studyType && (
                        <span className="inline-block mt-1.5 text-[10px] font-medium text-[#D97706] bg-[#FAF8F5] px-1.5 py-0.5 rounded border border-[#FAF8F5]">
                          {paper.studyType}
                        </span>
                      )}
                    </td>
                    ...
                    <td className="py-3.5 px-3.5 align-top font-mono text-[11px] text-[#431407] font-medium">
                      {sampleSizeDisplay}
                    </td>
                    ...
                    <td className="py-3.5 px-3.5 align-top text-center">
                      <span className="inline-block px-2 py-1 rounded text-[10px] font-medium bg-[#FFFFFF] text-[#431407] border border-[#FAF8F5] shadow-2xs">
      ...
      {selectedPaper && (
        <div className="bg-[#FFFFFF] border border-[#D97706]/30 rounded-[10px] p-5 shadow-xs space-y-3">
          <div className="flex items-center justify-between pb-2 border-b border-[#FAF8F5]">
            <div className="flex items-center gap-2">
              ...
              <h4 className="font-['DM_Sans'] text-sm font-semibold text-[#431407]">
                {selectedPaper.title}
              </h4>
            </div>
            {selectedPaper.url && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-xs text-[#D97706] hover:text-[#431407] inline-flex items-center gap-1 font-medium"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
            <div className="p-3 bg-[#FAF8F5] rounded border border-[#F4F4F5]">
              <span className="font-semibold text-[#431407] block mb-1">Architecture / Method:</span>
              <p className="text-[#4A5568] leading-relaxed">
                {selectedPaper.methodology || 'Not detailed'}
              </p>
            </div>
            <div className="p-3 bg-[#FAF8F5] rounded border border-[#F4F4F5]">
              <span className="font-semibold text-[#431407] block mb-1">Dataset / Cohort:</span>
              <p className="text-[#4A5568] leading-relaxed">
                {selectedPaper.population || selectedPaper.dataset || 'Not detailed'}
              </p>
            </div>
            <div className="p-3 bg-[#FAF8F5] rounded border border-[#F4F4F5]">
              <span className="font-semibold text-[#431407] block mb-1">Key Empirical Metrics:</span>
              <p className="text-[#4A5568] leading-relaxed">
