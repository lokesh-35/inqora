...
export default function App() {
  ...
  const [currentUser, setCurrentUser] = useState<UserProfile | null>(() => {
    try {
      const saved = localStorage.getItem('rexa_auth_user');
  ...
  const handleLoginSuccess = (user: UserProfile) => {
    setCurrentUser(user);
    try {
      localStorage.setItem('rexa_auth_user', JSON.stringify(user));
    } catch (e) {
  ...
  const handleLogout = () => {
    setCurrentUser(null);
    try {
      localStorage.removeItem('rexa_auth_user');
    } catch (e) {
  ...
  const handleSearch = async (options?: { isDeepReview?: boolean; prompt?: string }) => {
    ...
    try {
      ...
      if (effectiveQuery) {
        ...
        const searchRes = await fetch('/api/search-papers', {
          method: 'POST',
      ...
      try {
        const consensusRes = await fetch('/api/synthesize-consensus', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            question: effectiveQuery,
      ...
      if (options?.isDeepReview || filters.deepReviewMode) {
        ...
        const suggestRes = await fetch('/api/suggest-research', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
