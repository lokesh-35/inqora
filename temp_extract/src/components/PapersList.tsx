...
export const PapersList: React.FC<PapersListProps> = ({ papers }) => {
  ...
  const handleDownloadBibFile = () => {
    ...
    link.href = url;
    link.download = `rexa_citations_${Date.now()}.bib`;
  ...
  if (papers.length === 0) {
    return (
      <div className="bg-[#FFFFFF] rounded-xl border border-[#D9DEDA] p-10 text-center text-[#6B7280]">
        ...
        <p className="text-sm font-medium text-[#163A35]">No research papers indexed yet.</p>
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#D9DEDA]">
        <div className="text-xs text-[#6B7280] flex items-center gap-2">
          ...
          <span className="font-mono font-semibold text-[#163A35] bg-[#EFEFEA] px-2 py-0.5 rounded text-[11px]">
          ...
          >
            {copiedAll ? (
              <>
                ...
                <span className="text-[#2F6F68] font-semibold">Copied All (.bib)</span>
              </>
            ) : (
              <>
      ...
      {toastMessage && (
        <div className="bg-[#163A35] text-white text-xs px-4 py-2.5 rounded-md shadow-md flex items-center gap-2">
          <Check className="w-4 h-4 text-[#C7A66B]" />
        ...
        {papers.map((paper, index) => {
          ...
          return (
            <article
              ...
              className="group bg-[#FFFFFF] rounded-[10px] border border-[#D9DEDA] hover:border-[#2F6F68] hover:shadow-[0_4px_16px_rgba(22,58,53,0.06)] transition-all duration-200 overflow-hidden"
            >
              <div className="p-5 sm:p-6">
                {/* 1. Academic Header Metadata Line: Citation index, Year, Publication, Study Type, DOI */}
                <div className="flex flex-wrap items-center justify-between gap-y-1.5 gap-x-3 text-xs text-[#6B7280] pb-2 border-b border-[#F0F2EE]">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-mono text-[11px] font-semibold text-[#163A35] bg-[#EFEFEA] px-1.5 py-0.5 rounded">
                      [{index + 1}]
                    </span>
                    <span className="font-medium text-[#163A35]">{paper.year || '2024'}</span>
                    ...
                    {paper.studyType && (
                      <>
                        <span className="text-[#CBD5E1]">·</span>
                        <span className="text-[#2F6F68] font-medium text-[11px]">
                          {paper.studyType}
                        </span>
                      </>
                    )}
                    {displayDoi && (
                      <>
                        <span className="text-[#CBD5E1]">·</span>
                        <span className="font-mono text-[10px] text-[#6B7280] bg-[#F7F6F2] px-1.5 py-0.5 rounded border border-[#E5E7EB]">
                          DOI: {displayDoi}
                        </span>
                      </>
                    )}
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded text-xs font-medium border transition-colors cursor-pointer ${
                        copiedId === paper.id
                          ...
                          : 'bg-[#F7F6F2] text-[#163A35] border-[#D9DEDA] hover:border-[#2F6F68] hover:bg-[#FFFFFF]'
                      }`}
                      title="Copy BibTeX reference"
                    >
                      {copiedId === paper.id ? (
                        <>
                          <Check className="w-3 h-3 text-[#C7A66B]" />
                          <span>Copied</span>
                        </>
                      ) : (
                        <>
                          <Quote className="w-3 h-3 text-[#2F6F68]" />
                          <span>Cite</span>
                        </>
                      )}
                    </button>
                    ...
                    {paper.url && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="p-1 text-[#6B7280] hover:text-[#163A35] hover:bg-[#F7F6F2] rounded transition-colors"
                    ...
                    <button
                      ...
                      className="p-1 text-[#6B7280] hover:text-[#163A35] hover:bg-[#F7F6F2] rounded transition-colors cursor-pointer"
                ...
                <div className="mt-3">
                  <h3 className="font-['DM_Sans'] text-base sm:text-lg font-semibold text-[#163A35] leading-snug">
                    {paper.title}
                  </h3>
                ...
                <div className="mt-3.5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 py-2.5 px-3 bg-[#F7F6F2] rounded-lg border border-[#EAEAE4] text-xs">
                  <div>
                    <span className="text-[10px] uppercase font-semibold text-[#6B7280] tracking-wider block mb-0.5">
                      Sample Size
                    </span>
                    <span className="font-mono text-[11px] font-medium text-[#163A35] truncate block">
                      {displaySampleSize}
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-[#2F6F68] font-medium truncate block">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-[#163A35] font-medium truncate block">
                ...
                <div className="mt-3 p-3 bg-[#FFFFFF] border-l-2 border-[#2F6F68] rounded-r-md border-y border-r border-[#EAEAE4]">
                  <div className="text-[10px] font-bold text-[#2F6F68] uppercase tracking-wider mb-0.5 flex items-center gap-1.5">
                    <span>Key Finding</span>
                    <span className="text-[#C7A66B]">·</span>
                  ...
                  <div className="mt-3 pt-2.5 border-t border-[#F0F2EE]">
                    ...
                    <ul className="space-y-1 text-xs text-[#4A5568]">
                      {paper.limitations.slice(0, isExpanded ? paper.limitations.length : 2).map((lim, li) => (
                        <li key={li} className="flex items-start gap-2">
                          <span className="text-[#C7A66B] mt-0.5 font-bold">―</span>
                    ...
                    {paper.limitations.length > 2 && !isExpanded && (
                      <button
                        type="button"
                        onClick={() => toggleExpand(paper.id)}
                        className="text-[11px] text-[#2F6F68] hover:underline mt-1 font-medium cursor-pointer"
                      >
              ...
              {isExpanded && (
                <div className="px-5 sm:px-6 pb-5 pt-3 border-t border-[#D9DEDA] bg-[#F7F6F2]/70 space-y-3.5 text-xs">
                  {/* Abstract */}
                  {paper.abstract && (
                    <div>
                      <div className="font-semibold text-[#163A35] mb-1 flex items-center gap-1.5">
                        <BookOpen className="w-3.5 h-3.5 text-[#2F6F68]" />
                        <span>Abstract:</span>
                      </div>
                      <p className="text-[#4A5568] leading-relaxed bg-[#FFFFFF] p-3 rounded border border-[#D9DEDA]">
                        {paper.abstract}
                      </p>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {paper.methodology && (
                      <div className="bg-[#FFFFFF] p-3 rounded border border-[#D9DEDA]">
                        <span className="font-semibold text-[#163A35] block mb-1">Methodology:</span>
                    ...
                    {paper.evaluationMetrics && paper.evaluationMetrics.length > 0 && (
                      <div className="bg-[#FFFFFF] p-3 rounded border border-[#D9DEDA]">
                        <span className="font-semibold text-[#163A35] block mb-1">Evaluation Metrics:</span>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {paper.evaluationMetrics.map((metric, mi) => (
                            <span key={mi} className="font-mono text-[11px] bg-[#F7F6F2] px-2 py-0.5 rounded text-[#163A35] border border-[#D9DEDA]">
                              {metric}
                            </span>
                          ))}
                        </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <div className="font-semibold text-[#163A35] flex items-center gap-1.5">
                      ...
                      <button
                        type="button"
                        onClick={(e) => handleCopyBibTeX(paper, e)}
                        className="text-[11px] text-[#2F6F68] hover:underline font-medium cursor-pointer"
                      >
                        {copiedId === paper.id ? 'Copied' : 'Copy Entry'}
                      </button>
                    </div>
                    <pre className="p-3 bg-[#FFFFFF] border border-[#D9DEDA] rounded text-[11px] font-mono text-[#242A29] overflow-x-auto whitespace-pre">
