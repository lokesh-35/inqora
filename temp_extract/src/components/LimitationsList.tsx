      ...
      <div className="flex items-center justify-between pb-2 border-b border-[#D9DEDA]">
        <div>
          <h3 className="font-['DM_Sans'] text-base font-semibold text-[#163A35] flex items-center gap-2">
            <AlertOctagon className="w-4 h-4 text-[#C7A66B]" />
      ...
      <div className="space-y-4">
        {limitationGroups.map((group) => {
          ...
          return (
            <div
              key={group.id}
              className="bg-[#FFFFFF] rounded-[10px] border border-[#D9DEDA] shadow-2xs overflow-hidden"
            >
              <div className="p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-[#F7F6F2] text-[#2F6F68] border border-[#D9DEDA] mb-2">
                      {group.category}
                    </span>
                  ...
                  <button
                    ...
                    className="p-1.5 text-[#6B7280] hover:text-[#163A35] hover:bg-[#F7F6F2] rounded transition-colors cursor-pointer"
                ...
                <div className="mt-3 pt-3 border-t border-[#F0F2EE] flex flex-wrap items-center gap-2 text-xs">
                  ...
                  {group.supportingPaperTitles.map((title, idx) => (
                    <span
                      key={idx}
                      className="bg-[#F7F6F2] text-[#163A35] border border-[#D9DEDA] px-2 py-0.5 rounded text-[11px]"
                    >
                      {title}
                    </span>
                  ))}
              ...
              {isExpanded && group.evidence && group.evidence.length > 0 && (
                <div className="px-5 pb-5 pt-3 border-t border-[#D9DEDA] bg-[#F7F6F2] space-y-2">
                  <div className="text-[11px] font-semibold text-[#163A35] uppercase tracking-wider">
                  ...
                  <div className="space-y-2">
                    {group.evidence.map((ev, qi) => (
                      <div
                        key={qi}
                        className="bg-[#FFFFFF] border border-[#D9DEDA] rounded p-3 text-xs text-[#242A29] italic leading-relaxed flex items-start gap-2"
                      >
                        <Quote className="w-3.5 h-3.5 text-[#C7A66B] shrink-0 mt-0.5 not-italic" />
                        <div>
